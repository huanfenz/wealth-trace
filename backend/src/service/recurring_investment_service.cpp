...
void RecurringInvestmentService::execute(RecurringInvestmentExecution& e,
  ...
  e.status="FAILED"; e.transaction_id.reset(); e.failure_reason.reset();
  ...
  if(!source || !target) e.failure_reason="付款资产或股票基金已删除";
    ...
    Transaction tx;
    tx.household_id=source->household_id; tx.owner_member_id=source->owner_member_id;
    tx.type=TransactionType::Investment; tx.action=InvestmentAction::Buy;
    tx.transaction_time=date; tx.remark="股票基金定投"; tx.status=TransactionStatus::Normal;
    tx.created_at=now; tx.updated_at=now;
    tx.id=transactions_.create(tx);
    TransactionEntry out;
    out.transaction_id=tx.id;out.household_id=source->household_id;out.owner_member_id=source->owner_member_id;
    out.asset_id=source->id;out.direction=TransactionDirection::Out;out.amount=e.amount;
    out.balance_before=source->current_balance;out.balance_after=source_after;out.created_at=now;
    transactions_.add_entry(out);
    TransactionEntry in;
    in.transaction_id=tx.id;in.household_id=target->household_id;in.owner_member_id=target->owner_member_id;
    in.asset_id=target->id;in.direction=TransactionDirection::In;in.amount=e.amount;
    in.balance_before=target->current_balance;in.balance_after=target_after;in.created_at=now;
    transactions_.add_entry(in);
    transactions_.add_investment_detail(tx.id,target->id,InvestmentAction::Buy,e.amount);
    assets_.update_balance(source->id,source_after,now);assets_.update_balance(target->id,target_after,now);
    e.status="SUCCESS";e.transaction_id=tx.id;
  }
