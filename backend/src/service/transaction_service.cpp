...
Transaction TransactionService::write(std::int64_t household,const TransactionInput& input,std::optional<std::int64_t> id,bool rollback_assets){
  ...
  if(id){auto old=transactions_.find_by_id(*id);if(!old)throw not_found("transaction not found");tx=*old;
    if(tx.type!=input.type||tx.action!=input.action)throw invalid_request("transaction type and action cannot be changed");
    old_category_id=tx.category_id;
    old_entries=transactions_.entries(*id);
  }
  const bool entries_changed=!id||old_entries.size()!=input.entries.size()||
    !std::equal(old_entries.begin(),old_entries.end(),input.entries.begin(),input.entries.end(),
      [](const TransactionEntry& old,const TransactionEntryInput& next){return old.asset_id==next.asset_id&&old.direction==next.direction&&old.amount==next.amount;});
  ...
  if(input.preserve_legacy_category&&(!id||old_category_id||!tx.category||input.category_id))throw invalid_request("no legacy category to preserve");
  if(input.category_id){Statement c(database_,"SELECT name,household_id,type,active FROM transaction_category WHERE id=?;");c.bind(1,*input.category_id);if(!c.step())throw not_found("category not found");if(c.get_int64(1)!=household||c.get_text(2)!=to_string(input.type))throw invalid_request("category does not match household and transaction type");if(!c.get_int64(3)&&old_category_id!=input.category_id)throw conflict("category is inactive");tx.category=c.get_text(0);}else if(!input.preserve_legacy_category)tx.category.reset();
  if(id&&!entries_changed){TransactionGuard guard(database_);transactions_.update(tx);guard.commit();return tx;}
  ...
  for(std::size_t i=0;i<input.entries.size();++i){const auto& e=input.entries[i];auto a=assets_.find_by_id(e.asset_id);TransactionEntry row;row.transaction_id=tx.id;row.household_id=household;row.owner_member_id=a->owner_member_id;row.asset_id=a->id;row.direction=e.direction;row.amount=e.amount;row.created_at=now;
    if(!id||rollback_assets){const auto before=a->current_balance;const auto after=checked_add(before,entry_delta(e.direction,e.amount));assets_.update_balance(a->id,after,now);row.balance_before=before;row.balance_after=after;}
    transactions_.add_entry(row);}
...
TransactionDTO TransactionService::dto(std::int64_t id,bool include_entries){std::scoped_lock l(database_.mutex());auto tx=transactions_.find_by_id(id);if(!tx)throw not_found("transaction not found");TransactionDTO d;d.transaction=*tx;auto entries=transactions_.entries(id);if(include_entries)d.entries=entries;
  if(tx->type==TransactionType::Investment){Statement linked(database_,"SELECT 1 FROM recurring_investment_execution WHERE transaction_id=? LIMIT 1;");linked.bind(1,id);d.editable=!linked.step();}
  ...
  return d;}
Transaction TransactionService::update(std::int64_t id,const TransactionInput& input,bool rollback_assets){
  ...
  return write(old.household_id,input,id,rollback_assets);
}
